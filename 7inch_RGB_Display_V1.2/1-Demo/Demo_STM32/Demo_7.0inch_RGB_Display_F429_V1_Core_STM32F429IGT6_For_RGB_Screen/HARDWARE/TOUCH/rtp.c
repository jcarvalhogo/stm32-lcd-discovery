 ...
 * @retvalue   :None
******************************************************************************/  
void RTP_Drow_Touch_Point(u16 x,u16 y,u32 color)
{
	POINT_COLOR=color;
	LCD_DrawLine(x-12,y,x+13,y);//横线
	LCD_DrawLine(x,y-12,x,y+13);//竖线
	LCD_Draw_Point(x+1,y+1,POINT_COLOR);
	LCD_Draw_Point(x-1,y+1,POINT_COLOR);
	LCD_Draw_Point(x+1,y-1,POINT_COLOR);
	LCD_Draw_Point(x-1,y-1,POINT_COLOR);
 ...
 * @retvalue   :None
******************************************************************************/   
void RTP_Draw_Big_Point(u16 x,u16 y,u32 color)
{	    
	POINT_COLOR=color;
	LCD_Draw_Point(x,y,POINT_COLOR);//中心点 
	LCD_Draw_Point(x+1,y,POINT_COLOR);
	LCD_Draw_Point(x,y+1,POINT_COLOR);
	LCD_Draw_Point(x+1,y+1,POINT_COLOR);	 	  	
}	
...
{	
	...
	if(touch_flag)
	{
		irq=HAL_GPIO_ReadPin(GPIOH, GPIO_PIN_7);
	}
	if(!irq)
#else		   
	if(PEN==0)//有按键按下
		...
		{	
			...
			{			   
				case 1:						 
					...
					RTP_Drow_Touch_Point(lcd_dev.width-20,20,RED);	//画点2
					break;
				case 2:
 					RTP_Drow_Touch_Point(lcd_dev.width-20,20,WHITE);	//清除点2
					RTP_Drow_Touch_Point(20,lcd_dev.height-20,RED);	//画点3
					break;
				case 3:
 					RTP_Drow_Touch_Point(20,lcd_dev.height-20,WHITE);			//清除点3
 					RTP_Drow_Touch_Point(lcd_dev.width-20,lcd_dev.height-20,RED);	//画点4
					break;
				case 4:	 //全部四个点已经得到
	    		    //对边相等
					...
					{
						cnt=0;
 				    	RTP_Drow_Touch_Point(lcd_dev.width-20,lcd_dev.height-20,WHITE);	//清除点4
   	 					RTP_Drow_Touch_Point(20,20,RED);								//画点1
					...
					//计算结果
					tp_dev.xfac=(float)(lcd_dev.width-40)/(pos_temp[1][0]-pos_temp[0][0]);//得到xfac		 
					tp_dev.xoff=(lcd_dev.width-tp_dev.xfac*(pos_temp[1][0]+pos_temp[0][0]))/2;//得到xoff
						  
					tp_dev.yfac=(float)(lcd_dev.height-40)/(pos_temp[2][1]-pos_temp[0][1]);//得到yfac
					tp_dev.yoff=(lcd_dev.height-tp_dev.yfac*(pos_temp[2][1]+pos_temp[0][1]))/2;//得到yoff  
