...
u32 POINT_COLOR = 0x00000000,BACK_COLOR = 0xFFFFFFFF;  
...
{ 
	...
	{
		case 0:
			*(u16*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*(rgblcd.pheight-x-1)+y))=color&0x00FFFF;
			*(u8*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*(rgblcd.pheight-x-1)+y)+2)=(color&0xFF0000)>>16;		
		break;
		case 1:
			*(u16*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*y+x))=color&0x00FFFF;
			*(u8*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*y+x)+2)=(color&0xFF0000)>>16;
		break;
		case 2:
			*(u16*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*x+rgblcd.pwidth-y-1))=color&0x00FFFF;
			*(u8*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*x+rgblcd.pwidth-y-1)+2)=(color&0xFF0000)>>16;
		break;
		case 3:
			*(u16*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*(rgblcd.pheight-y-1)+rgblcd.pwidth-x-1))=color&0x00FFFF;
			*(u8*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*(rgblcd.pheight-y-1)+rgblcd.pwidth-x-1)+2)=(color&0xFF0000)>>16;
		break;
	}
#else
	switch(rgblcd.dir)
...
{ 
	...
	switch(rgblcd.dir)
	{
		case 0:
			...
			color	|= *(u8*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*(rgblcd.pheight-x-1)+y)+2)<<16;
		break;
		case 1:
			color	= *(u16*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*y+x));
			color	|= *(u8*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*y+x)+2)<<16;
		break;
		case 2:
			color	= *(u16*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*x+rgblcd.pwidth-y-1));
			color	|= *(u8*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*x+rgblcd.pwidth-y-1)+2)<<16;
		break;
		case 3:
			color	= *(u16*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*(rgblcd.pheight-y-1)+rgblcd.pwidth-x-1));
			color	|= *(u8*)((u32)rgblcd_framebuf[rgblcd.activelayer]+rgblcd.pixsize*(rgblcd.pwidth*(rgblcd.pheight-y-1)+rgblcd.pwidth-x-1)+2)<<16;
		break;
	}	
#else
	switch(rgblcd.dir)
...
{
	LCD_Fill_color(0,0,rgblcd.width-1,rgblcd.height-1,color);
}
...
//返回值:0,成功;1,失败。
u8 LCD_Clk_Set(u32 pllsain,u32 pllsair,u32 pllsaidivr)
{
...
*/
    __HAL_RCC_GPIOG_CLK_ENABLE();       //使能GPIOG时钟
	__HAL_RCC_GPIOI_CLK_ENABLE();       //使能GPIOI时钟
	__HAL_RCC_GPIOB_CLK_ENABLE();       //使能GPIOG时钟
    ...
    GPIO_Initure.Pin=GPIO_PIN_2; //PI2,7
    HAL_GPIO_Init(GPIOI,&GPIO_Initure);     //初始化
		
    GPIO_Initure.Pin=GPIO_PIN_9; //PI2,7
...
//	u16 lcdid=0;
	...
	lcd_dev.id=LCD_Read_ID();			//读取LCD面板ID	
	if(lcd_dev.id==0X7584)
	{
		...
		rgblcd.vfp=22;				    //垂直前廊
#if LCD_PIXFORMAT==LCD_PIXFORMAT_ARGB8888
		LCD_Clk_Set(370,4,RCC_PLLSAIDIVR_4);			//设置像素时钟 16.5M(如果开双显,需要降低DCLK到:18.75Mhz  pll3r=43,才会比较好)
#elif LCD_PIXFORMAT==LCD_PIXFORMAT_RGB888
		LCD_Clk_Set(384,6,RCC_PLLSAIDIVR_4);			//设置像素时钟 16.5M(如果开双显,需要降低DCLK到:18.75Mhz  pll3r=43,才会比较好)
#else
		LCD_Clk_Set(396,3,RCC_PLLSAIDIVR_4);			//设置像素时钟 33M(如果开双显,需要降低DCLK到:18.75Mhz  pll3r=43,才会比较好)
#endif
	}else if(lcd_dev.id==0X7016)		
	{
    ...
    rgblcd.hsw=20;				    //水平同步宽度
		...
		rgblcd.hfp=160;			    //水平前廊
		rgblcd.vfp=12;				    //垂直前廊
		LCD_Clk_Set(360,2,RCC_PLLSAIDIVR_4);			//设置像素时钟  45Mhz 
		//其他参数待定.
	}
   
#if LCD_PIXFORMAT==LCD_PIXFORMAT_ARGB8888
	rgblcd_framebuf[0]=(u32*)&rgb_lcd_framebuf;
	rgblcd.pixsize=4;				//每个像素占4个字节

#elif LCD_PIXFORMAT==LCD_PIXFORMAT_RGB888
	rgblcd_framebuf[0]=(u32*)&rgb_lcd_framebuf;
	rgblcd.pixsize=3;				//每个像素占3个字节
  ...
  rgblcd.pixsize=2;				//每个像素占2个字节
	...
	//层配置
	LCD_Layer_Parameter_Config(0,(u32)rgblcd_framebuf[0],LCD_PIXFORMAT,255,0,4,5,0X000000);//层参数配置
	LCD_Layer_Window_Config(0,0,0,rgblcd.pwidth,rgblcd.pheight);	//层窗口配置,以LCD面板坐标系为基准,不要随便修改!	
	 	
 	LCD_Set_Direction(USE_HORIZONTAL);			//设置显示方向
	LCD_Select_Layer(0); 			//选择第1层
  LCD_LED=1;         		    //点亮背光
	LCD_DISP=1;
...
void HAL_LTDC_MspInit(LTDC_HandleTypeDef* hltdc)
{
    ...
    //初始化PD7，背光引脚
    GPIO_Initure.Pin=GPIO_PIN_4|GPIO_PIN_7;                //PD4、7推挽输出，控制显示和背光
    ...
    //初始化PA3,11,12
    GPIO_Initure.Pin=GPIO_PIN_3|GPIO_PIN_11|GPIO_PIN_12; 
    GPIO_Initure.Mode=GPIO_MODE_AF_PP;          //复用
    GPIO_Initure.Pull=GPIO_PULLUP;              
    GPIO_Initure.Speed=GPIO_SPEED_HIGH; //高速
    ...
    //初始化PH2,3,8,13,15
    GPIO_Initure.Pin=GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_8|GPIO_PIN_13|GPIO_PIN_15;
    HAL_GPIO_Init(GPIOH,&GPIO_Initure);
    
    //初始化PI0,2,4,9,10
    GPIO_Initure.Pin=GPIO_PIN_0|GPIO_PIN_2|GPIO_PIN_4|GPIO_PIN_9|GPIO_PIN_10;
