 ...
 * @retvalue   :None
******************************************************************************/	
void MCULCD_GPIO_Init(FMC_NORSRAM_TimingTypeDef FWT)
{
		GPIO_InitTypeDef GPIO_Initure;
...
//    FMC_NORSRAM_TimingTypeDef FSMC_WriteTim;
    ...
    //FSMC读时序控制寄存器
    FSMC_ReadWriteTim.AddressSetupTime=0x0F;    //地址建立时间(ADDSET)为17个HCLK 1/216M=4.6ns*17=78ns
    FSMC_ReadWriteTim.AddressHoldTime=0x00;
    FSMC_ReadWriteTim.DataSetupTime=0x46;       //数据保存时间(DATAST)为85个HCLK	=4.6*85=391ns
    FSMC_ReadWriteTim.AccessMode=FMC_ACCESS_MODE_A; //模式A
    //FSMC写时序控制寄存器
    FWT.AddressSetupTime=0x0F;        //地址建立时间(ADDSET)为21个HCLK=96ns
    FWT.AddressHoldTime=0x00;
    FWT.DataSetupTime=0x0F;           //数据保存时间(DATAST)为4.6ns*21个HCLK=96ns
...
void MCULCD_FSMC_Write_Time_Set(FMC_NORSRAM_TimingTypeDef FWT, uint32_t AST, uint32_t DST)
{
...
{ 
	FMC_NORSRAM_TimingTypeDef FSMC_WriteTim;
	MCULCD_GPIO_Init(FSMC_WriteTim);//LCD GPIO初始化	
	delay_ms(100);
...
//************* SSD1963初始化**********//	
	...
	//重新配置写时序控制寄存器的时序 
	MCULCD_FSMC_Write_Time_Set(FSMC_WriteTim, 4, 4);
